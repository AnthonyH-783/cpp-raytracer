  ...
  // Constructor to initialize dielectric material with refractive index
    ...
    const override {
        ...
        vec3 direction;
         ...
         if (cannot_refract || reflectance(cos_theta, ri) > random_double()) // Reflect the ray
         
            direction = reflect(unit_direction, rec.normal);
        else // Otherwise, refract the ray
            direction = refract(unit_direction, rec.normal, ri);

        scattered = ray(rec.p, direction); // Create scattered ray
        return true;
    }
